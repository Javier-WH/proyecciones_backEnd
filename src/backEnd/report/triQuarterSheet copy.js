...
export default function generateTriQuarterSheet ({
  ...
  for (const pnf of pnfArray) {
    ...
    let row = 1
    // encabezado
    const rangeLine1 = sheet.range(`A${row}:M${row}`)
    ...
    const rangeLine2 = sheet.range(`A${row}:M${row}`)
    ...
    const rangeLine3 = sheet.range(`A${row}:M${row}`)
    ...
    const rangeLine4 = sheet.range(`A${row}:M${row}`)
    ...
    const rangeLine5 = sheet.range(`A${row}:M${row}`)
    ...
    sheet.cell(`F${row}`).value('TRIMESTRE I')
    ...
    const rangeLine6HI = sheet.range(`H${row}:I${row}`)
    sheet.cell(`H${row}`).value('TRIMESTRE II')
    ...
    sheet.cell(`H${row + 1}`).value('Horas por U/C')
    ...
    sheet.cell(`I${row + 1}`).value('Total de Horas')
    ...
    const rangeLine6JK = sheet.range(`J${row}:K${row}`)
    sheet.cell(`J${row}`).value('TRIMESTRE III')
    ...
    sheet.cell(`J${row + 1}`).value('Horas por U/C')
    ...
    sheet.cell(`K${row + 1}`).value('Total de Horas')
    ...
    const rangeLine6H = sheet.range(`L${row}:L${row + 1}`)
    sheet.cell(`L${row}`).value('Dedicación')
    ...
    const rangeLine6I = sheet.range(`M${row}:M${row + 1}`)
    sheet.cell(`M${row}`).value('Observación')
    ...
    for (const [teacherIndex, teacher] of teachers.entries()) {
      ...
      for (const [subjectIndex, subject] of teacher.load.entries()) {
        ...
        sheet.cell(`L${row}`).value(teacherContractType)
      ...
      const totalHourCellRangeI = sheet.range(`G${initRange}:G${initRange + teacher.load.length - 1}`)
      ...
      const totalHourCellRangeII = sheet.range(`I${initRange}:I${initRange + teacher.load.length - 1}`)
      ...
      const totalHourCellRangeIII = sheet.range(`K${initRange}:K${initRange + teacher.load.length - 1}`)
      ...
      const dedicationCellRange = sheet.range(`L${initRange}:L${initRange + teacher.load.length - 1}`)
      ...
      const observationCellRange = sheet.range(`M${initRange}:M${initRange + teacher.load.length - 1}`)
