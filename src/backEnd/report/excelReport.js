...
export async function generateExcelReport (req, res) {
  try {
    ...
    const { sheetNumber, workbook: singleQuaterWarkbook } = generateSingleQuarterSheet({ sheetNumber: 0, workbook, pnfArray: groupedByProgram, proyectionDate })

    singleQuaterWarkbook.toFileAsync('C:/Users/Axioma/Desktop/report.xlsx')

    return res.status(200).json({ message: 'Proyección activa' })
